    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                student_proj = torch.cat([f[mask_lab].unsqueeze(1) for f in student_proj.chunk(args.n_views)], dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='vedai', help='options: cifar10, cifar100, imagenet_100, cub, scars, fgvc_aricraft, herbarium_19')
    ...
    init_experiment(args, runner_name=['ncdir'])
    ...
    if 'vedai' in pretrain_path:
        for key in list(state_dict["state_dict"].keys()):
            if 'backbone.' in key:
                newKey = key.replace('backbone.', '')
                newKey = newKey.replace('layers', 'blocks')
                newKey = newKey.replace('projection', 'proj')
                newKey = newKey.replace('ln1', 'norm1')
                newKey = newKey.replace('ln2', 'norm2')
                newKey = newKey.replace('ffn.blocks.0.0', 'mlp.fc1')
                newKey = newKey.replace('ffn.blocks.1', 'mlp.fc2')
                state_dict["state_dict"][newKey] = state_dict["state_dict"].pop(key)
            else:
                state_dict["state_dict"].pop(key)
        state_dict["state_dict"]["norm.weight"] = state_dict["state_dict"].pop("norm1.weight")
        state_dict["state_dict"]["norm.bias"] = state_dict["state_dict"].pop("norm1.bias")
